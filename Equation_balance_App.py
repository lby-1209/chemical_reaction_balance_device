...
class kivyGrid(GridLayout):
    def __init__(self, **kwargs):
        ...
        self.add_widget(Label(text = '화학반응식 입력 괄호 뒤 원자수 표시', font_name = KOREAN_FONT, font_size = 30))
        self.add_widget(Label(text = '입력예시 : (H)2 + (O)2 -> (H)2(O)1', font_name = KOREAN_FONT, font_size = 30))    
        
        self.user_input = TextInput(multiline = False, font_name = KOREAN_FONT, font_size = 30)
        self.add_widget(self.user_input)
        
        self.Equation_balance_Button = Button(text = '실행', font_name = KOREAN_FONT, font_size = 30)
        self.Equation_balance_Button.bind(on_press = self.button_click_callback)
        self.add_widget(self.Equation_balance_Button)
        
    def button_click_callback(self, instance):
        user_input = self.user_input.text      
        
        chemistry_Equation_balance_end = Equation(user_input)
        chemistry_Equation_balance_end = chemistry_Equation_balance_end.balance()
        chemistry_Equation_balance_end = str(chemistry_Equation_balance_end)        
           
        chemistry_Equation_balance_end_label = Label(text = '화학반응식 완결 : ' + chemistry_Equation_balance_end, font_name = KOREAN_FONT, font_size = 30)
        self.add_widget(chemistry_Equation_balance_end_label)

class chemistry_Equation_balance_App(App):
    def build(self):
        return kivyGrid()

if __name__ == '__main__':
    chemistry_Equation_balance_App().run()
